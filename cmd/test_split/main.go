// Command test_split performs a CTF splitPosition transaction.
//
// Supports two modes:
//   - Direct on-chain (requires MATIC for gas, uses --rpc-url):
//     go run ./cmd/test_split/ --slug <slug> --amount 5
//   - Gasless relayer (no MATIC needed, uses RELAYER_API_KEY):
//     go run ./cmd/test_split/ --slug <slug> --amount 5 --relayer
//
...
//	--amount <number>      USDC amount to split (required, e.g. 10 = $10)
...
func loadConfig() config {
	...
	flag.Float64Var(&cfg.AmountUSDC, "amount", 0, "Amount of USDC to split (required, e.g. 10 = $10)")
...
func main() {
	...
	// ── Build request ───────────────────────────────────────
	req := &polymarket.SplitPositionRequest{
	...
	// ── Execute split ───────────────────────────────────────
	var resp *polymarket.SplitPositionResponse
	...
	if cfg.Relayer {
		...
		// Step 1: Check allowance — does the (neg-risk) collateral adapter have approval
		// to spend the proxy wallet's pUSD?
		fmt.Printf("🔍 Checking allowance for adapter (%s)... ", adapter.Hex())
		allowance, checkErr := checkAllowance(context.Background(), cfg.RPCURL, collateral, adapter, signerAddr)
		if checkErr != nil {
			fmt.Fprintf(os.Stderr, "\n❌ Allowance check failed: %v\n", checkErr)
			os.Exit(1)
		}
		fmt.Printf("%s raw (need %s)\n", allowance.String(), amount.String())

		if allowance.Cmp(amount) < 0 {
			fmt.Println("⚡ Allowance insufficient — submitting approve via relayer...")
			appResp, appErr := relayer.ApproveTokenViaRelayer(
				context.Background(),
				collateral,
				adapter,
				new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 256), big.NewInt(1)), // max
				privateKey,
			)
			if appErr != nil {
			...
			fmt.Printf("✅ Approve confirmed: %s\n", appResp.TransactionHash.Hex())
		} else {
			fmt.Println("✅ Allowance sufficient — skipping approve")
		}

		// Step 2: Submit splitPosition via relayer
		fmt.Println("⏳ Submitting splitPosition via relayer...")
		resp, err = relayer.SplitPositionViaRelayer(context.Background(), req, privateKey, market.NegRisk)
	} else {
		fmt.Println("⏳ Sending direct on-chain splitPosition transaction...")
		...
		resp, err = ctfClient.SplitPosition(context.Background(), req, privateKey)
	}

	if err != nil {
		fmt.Fprintf(os.Stderr, "❌ SplitPosition failed: %v\n", err)
		os.Exit(1)
	}
	...
	fmt.Println(strings.Repeat("─", 60))
	fmt.Println("\n📝 You can now trade the YES/NO tokens on the CLOB.")
	fmt.Println("   Use --asset-id <token-id> with test_market to trade them.")
}
...
// checkAllowance queries the ERC-20 allowance for proxy wallet → spender via RPC.
func checkAllowance(ctx context.Context, rpcURL string, token, spender common.Address, signerAddr common.Address) (*big.Int, error) {
	...
	proxyWallet := polymarket.DeriveProxyWallet(signerAddr)

	// ERC-20 allowance(address owner, address spender) → uint256
	// selector: 0xdd62ed3e
	allowanceSelector := common.Hex2Bytes("dd62ed3e")
	ownerPadded := common.LeftPadBytes(proxyWallet.Bytes(), 32)
	spenderPadded := common.LeftPadBytes(spender.Bytes(), 32)
	callData := append(allowanceSelector, ownerPadded...)
	callData = append(callData, spenderPadded...)
